...
class FightPage(QWidget):
    ...
    def _init_ui(self):
        ...
        # 标题
        title = QLabel("模块一：打斗识别")
        title.setStyleSheet("font-size: 18px; font-weight: bold;")
        layout.addWidget(title)

        desc = QLabel("自动识别视频中的打斗片段，支持多人同框校验，"
                      "可批量导出或生成剪映草稿。")
    ...
    def _start(self):
        ...
        from module_a_fight import FightPipeline
        pipeline = FightPipeline()
        ...
        if task.status.value in ("done", "error", "canceled"):
            ...
            if task.status.value == "done":
                ...
                QMessageBox.information(
                    self, "完成",
                    f"识别完成，共 {task.result.get('total', 0)} 个片段")
    ...
    def _export_jianying(self):
        ...
        from module_a_fight import FightPipeline
        pipeline = FightPipeline()
        result = pipeline.export_jianying(clips, "打斗片段")
        if result.get("success"):
            QMessageBox.information(
                self, "成功",
                f"剪映草稿已生成:\n{result.get('draft_dir')}\n"
                f"片段数: {result.get('clip_count')}")
        else:
